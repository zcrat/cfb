        ...
        $(function() {
            ...
            function showElements() {
                ...
                $.each(paginatedElements, function(index, element) {
                    let row = $('<tr>');
                    row.append('<td><div class="Datatable-content">' + (element.folioNum ? element
                        .folioNum : "Sin #ORDEN") + '</div></td>');
                    row.append('<td><div class="Datatable-content">' + (element.orden_seguimiento ? element
                        .orden_seguimiento : "Sin # Seguimiento") + '</div></td>');
                    row.append('<td><div class="Datatable-content">' + (element.folio ? element.folio :
                        "folio") + '</div></td></tr>');
                    row.append('<td><div class="Datatable-content">' + (element.empresa ? element.empresa
                        .nombre : "Sin Empresa") + '</div></td></tr>');
                    row.append('<td><div class="Datatable-content">' +
                        (element.vehiculo ? ((element.vehiculo.marca ? element.vehiculo.marca.nombre :
                                "Sin Marca") + '--' + (element.vehiculo.modelo ? element.vehiculo
                                .modelo.nombre : "Sin Marca") + '--' + (element.vehiculo.placas ?
                                element.vehiculo.placas : "Sin Placas")) :
                            "El vehicuylo no tiene datos") +
                        '</div></td></tr>');
                    row.append('<td><div class="Datatable-content">' + (element.fecha ? element.fecha :
                        "No Se Registro") + '</div></td></tr>');
                    row.append('<td><div class="Datatable-content">' + (element.fecha_compromiso ? element
                        .fecha_compromiso : "No Se Registro") + '</div></td></tr>');
                    row.append('<td><div class="Datatable-content">' +
                        '<button class="btn btn-success reporte" onclick="executereporte(' + (element
                            .id ? element.id : 1) +
                        ...
                        '<button class="btn btn-danger" onclick="executedelete(' + (element.id ? element
                            .id : 1) +
                        ')"><i aria-hidden="true" class="fa-solid fa-trash"></i></button>' +
                        '<button class="btn btn-info" onclick="executeagregarservicio(\'' + (element
                            .folioNum ? element.folioNum : 1) + '\',' + (element.empresa ? element
                            .empresa.id : 1) +
            ...
            function filtering() {
                ...
                if (elements.length === 0) {
                    ...
                    if (search !== '' && option1 === '') {
                        ...
                        $('#no-results-message').text(
                            'No Se Encontraron Recepcion Vehiculares Con #Orden, Ord. Seguimmiento, Folio, Marca, Modelo o Placas Que Coincidan Con  ' +
                            search + ' de la Empresa ' + nombreempresa);
                    }
                } else {
