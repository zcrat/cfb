...
<div class="modal fade" id="recepcionservicioyconceptos" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" role="dialog" aria-labelledby="recepcionservicioLabel" >
    <div class="modal-dialog zdmw-95pct modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="vaniflex zdmg-r05 zdjc-between zdfw-w">
                <div class="zdw-35pct vaniflex zdfd-column">
                ...
                <div class="zdw-30pct vaniflex zdfd-column">
                    <div class="vaniflex zdfd-column"> 
                        ...
                        <label class="ImporteConceptos" id="subtotalcosto" for="">Costos: $0</label>
...
<script>
    ...
    window.executepresupuestoaprobar = function(id) {
        $.ajax({
            type: 'GET',
   ...
   window.reloadlista = function(id){
    ...
    function actualizarlista(){
        ...
        $.each(listaconceptos, function(index, element) {
            ...
            row.append('<td><div class="Datatable-content"><input type="number" class="preciodiagnosticofinal zdw-r4" data-id="'+element.id+'" value='+element.precio_v+'></input></div></div></td>');
            //row.append('<td><div class="Datatable-content">' + (element.precio_v ? element.precio_v : 0 ) + '</div></td>');
            row.append('<td><label data-id="' + element.id + '" class="subtotaldiagnostico ">' + Number((element.cantidad * element.precio_v)).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '</label></td>');
            ...
            let subtotalcosto = 0;
                listaconceptos.forEach(item => {
                    subtotalcosto+=item.cantidad * item.precio;
                    subtotalventa += item.cantidad * item.precio_v;
            });
            ...
            /* 
            console.log(total); // 22746.625600000003
            console.log(totalRedondeado); // 22746.63
            console.log(totalFormateado); // 22
            $('#subtotalcosto').text("Costos: $").append(Number(subtotalcosto).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
            $('#subtotaldiagnostico').text("Subtotal: $").append(Number(subtotal).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
            $('#ivadiagnostico').text("Iva:      $").append(Number(iva).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
            $('#totaldiagnostico').text("Total:    $").append(Number(total).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })); */
        ...
        console.log("secambia")
            ...
            if (itemIndex !== -1) {
                ...
                if ($(this).hasClass('preciodiagnostico')) {
                    ...
                    listaconceptos[itemIndex].precio_v = val;
